...
--
function optim.lswolfe_cuda(opfunc,x,t,d,f,g,gtd,options)
   -- options
   options = options or {}
   ...
   x:add(t,d:cuda()) -- *** added :cuda()
   local f_new,g_new = opfunc(x);    g_new = g_new:float()  -- *** added :float() 
   local lsFuncEval = 1
   ...
   while LSiter < maxIter do
      ...
      x[{}] = x_init
      x:add(t,d:cuda())   -- *** added :cuda()
      f_new,g_new = opfunc(x);  g_new = g_new:float()   -- *** added :float()
